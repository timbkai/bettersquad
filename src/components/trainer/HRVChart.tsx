"use client";

import {
  AreaChart,
  Area,
  XAxis,
  YAxis,
  CartesianGrid,
  Tooltip,
  ResponsiveContainer,
  ReferenceLine,
} from "recharts";
import type { HRVEntry } from "@/data/mockData";

interface HRVChartProps {
  data: HRVEntry[];
}

const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-slate-900 border border-white/10 rounded-lg p-3 text-xs shadow-xl">
        <div className="text-slate-400 mb-1">{label}</div>
        <div className="font-bold text-red-400">{payload[0].value} ms</div>
      </div>
    );
  }
  return null;
};

export default function HRVChart({ data }: HRVChartProps) {
  const avg = Math.round(data.reduce((s, d) => s + d.hrv, 0) / data.length);
  const formatted = data.map((d) => ({ ...d, date: d.date.slice(5) }));

  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <span className="text-sm font-semibold text-slate-300">HRV-Trend (14 Tage)</span>
        <span className="text-sm text-slate-400">Ø <strong className="text-white">{avg} ms</strong></span>
      </div>
      <ResponsiveContainer width="100%" height={160}>
        <AreaChart data={formatted} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
          <defs>
            <linearGradient id="hrvGrad" x1="0" y1="0" x2="0" y2="1">
              <stop offset="5%" stopColor="#ef4444" stopOpacity={0.25} />
              <stop offset="95%" stopColor="#ef4444" stopOpacity={0} />
            </linearGradient>
          </defs>
          <CartesianGrid strokeDasharray="3 3" />
          <ReferenceLine y={avg} stroke="rgba(239,68,68,0.4)" strokeDasharray="4 4" />
          <XAxis dataKey="date" tick={{ fontSize: 10, fill: "#64748b" }} tickLine={false} axisLine={false} />
          <YAxis domain={[45, 100]} tick={{ fontSize: 10, fill: "#64748b" }} tickLine={false} axisLine={false} />
          <Tooltip content={<CustomTooltip />} />
          <Area
            type="monotone"
            dataKey="hrv"
            stroke="#ef4444"
            strokeWidth={2}
            fill="url(#hrvGrad)"
            dot={false}
            activeDot={{ r: 4, fill: "#ef4444" }}
          />
        </AreaChart>
      </ResponsiveContainer>
      <div className="text-xs text-slate-500 mt-1">HRV (ms) · Polar H10 · RMSSD</div>
    </div>
  );
}
